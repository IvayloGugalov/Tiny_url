  ...
  try {
    ...
    if (loginResponse.ok && loginData.token) {
      ...
      if (analyticsResponse.ok && analyticsData.data) {
        ...
        const hasAuthenticatedLink = analyticsData.data.some(link => 
          link.target === 'https://authenticated-link.com'
        );
        const hasAnonymousLink = analyticsData.data.some(link => 
          link.target === 'https://anonymous-link.com'
        );
        ...
        if (hasAuthenticatedLink && !hasAnonymousLink) {
          ...
          console.log('❌ Analytics shows both user and anonymous links (should only show user links)');
        } else {
