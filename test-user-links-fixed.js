    ...
    if (loginResponse.ok && loginData.token) {
      ...
      if (analyticsResponse.ok && analyticsData.data) {
        ...
        analyticsData.data.forEach((link, index) => {
          console.log(`   ${index + 1}. ${link.id} -> ${link.target} (${link.clicks} clicks)`);
        });

        // Check if only authenticated user's links are returned
        const hasUserLink1 = analyticsData.data.some(link => 
          link.target === 'https://user-specific-link.com'
        );
        const hasUserLink2 = analyticsData.data.some(link => 
          link.target === 'https://another-user-link.com'
        );
        const hasAnonymousLink = analyticsData.data.some(link => 
          link.target === 'https://anonymous-test.com'
        );
        ...
        if (hasUserLink1 && hasUserLink2 && !hasAnonymousLink) {
          ...
          console.log('❌ Analytics shows anonymous links (should only show user links)');
        } else {
